...
BOOL(WINAPI* TrueDuplicateHandle)(HANDLE, HANDLE, HANDLE, LPHANDLE, DWORD, BOOL, DWORD) = DuplicateHandle;
...
int blockedCount = 0;
int totalPolicies = 16;  // Update this with more policies
...
LPVOID WINAPI HookedVirtualAlloc(LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect) {
    std::wcout << L"Attempting to allocate memory." << std::endl;
    // Block if trying to allocate executable memory
    if (flProtect & PAGE_EXECUTE_READWRITE) {
        std::wcout << L"Blocked: Executable memory allocation not allowed!" << std::endl;
        blockedCount++;
        return NULL;
    }
...
BOOL WINAPI HookedReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped) {
    ...
    // Mock logic for sensitive file
    LPCWSTR sensitiveFile = L"sensitive.txt";
    HANDLE sensitiveFileHandle = CreateFileW(sensitiveFile, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

    if (hFile == sensitiveFileHandle) {  // Check if the file handle corresponds to sensitive.txt
        std::wcout << L"Blocked: Reading from sensitive file!" << std::endl;
        blockedCount++;
        CloseHandle(sensitiveFileHandle);  // Don't forget to close the handle
        return FALSE;  // Block the operation
    }

    CloseHandle(sensitiveFileHandle);
...
HANDLE WINAPI HookedCreateFileA(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) {
    std::cout << "Attempting to create or open file: " << lpFileName << std::endl;
    // Block specific files (e.g., "blocked_file.txt")
    if (strstr(lpFileName, "blocked_file.txt") != NULL) {
        std::cout << "Blocked: File creation or access is not allowed for this file!" << std::endl;
        blockedCount++;
        return INVALID_HANDLE_VALUE;
    }
