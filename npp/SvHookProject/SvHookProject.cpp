...
"C:/Users/TAQI SHAH/Desktop/hooks/shellcodes/windows_x64_custom_bind_tcp.bin",
    ...
    for (const auto& shellcodeFile : shellcodeList) {
        ...
        if (shellcode) {
            blockedCount = 0;  // Reset blocked count before testing
    ...
    // Print shellcode test results
    for (const auto& result : results) {
        std::cout << "Shellcode: " << result.shellcodeFile << " - " << (result.blocked ? "Blocked" : "Allowed") << std::endl;
    }
